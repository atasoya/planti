...
interface HumidityCompareProps {
  plantId?: number;
}
...
export function HumidityCompare({ plantId }: HumidityCompareProps) {
  ...
  useEffect(() => {
    async function fetchHumidityData() {
      if (!plantId) {
        setHumidityData(generateDemoData());
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const optimalHumidity = plant.humidity;

        const formattedData = data.history
          .sort(
            (a: HumidityRecord, b: HumidityRecord) =>
              new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()
          )
          .map((record: HumidityRecord) => ({
  ...
  const generateDemoData = (): HumidityData[] => {
    ...
    const optimalHumidity = 80;

    while (currentDate <= endDate) {
      const actualHumidity = Math.floor(Math.random() * 30) + 60;
  ...
  const filteredData = humidityData.filter((item) => {
  ...
  if (error && humidityData.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Humidity Comparison</CardTitle>
    ...
    return (
      <Card>
        ...
        <CardContent className="h-[250px] flex items-center justify-center text-gray-500 text-center px-4">
          Not enough humidity data available. At least 7 data points are needed
    ...
    <Card>
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1 text-center sm:text-left">
          <CardTitle>Humidity Comparison</CardTitle>
          <CardDescription>
            Comparing optimal vs. actual humidity levels
          </CardDescription>
        </div>
